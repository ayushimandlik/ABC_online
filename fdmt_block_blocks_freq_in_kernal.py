...
#        ohdr['_tensor']['shape'][-1]  = 256
        ...
        self.td = bf.ndarray(shape = (1, 512, 1), dtype = int, space = 'cuda')
        ...
        if self.n_iter >= 2:
            ...
            for ob_index, beam_bl in enumerate(beam_blocks):
                ...
                for beam_num in range(3):
                    print(self.block_full[0, (3 * beam_bl[0]) + beam_num, :, self.timechunksize : self.timechunksize * 2 ].shape)
...
#                    self.f.execute(self.block_full[0, (3 * beam_bl[0]) + beam_num, :, self.timechunksize : self.timechunksize * 2 ], self.fdmt_block[0, (3 * ob_index) + beam_num, :, :], negative_delays = True) 
                ...
                delay_samples = np.zeros((1, 512, 1), dtype = int)
                for i in range(512):
                ...
                if beam > 1 and beam < 21:
                    ...
                    np.save('candidate', self.candidate[0, ob_index, :, :, :])

...
#            self.block_N_1 = bf.copy_array(ispan.data, space = 'cuda')
        if self.n_iter >= 2:
            self.predictions = np.load('Direct_classifier_logs_' + str(self.n_iter - 2) + '.npy')
            ...
            idata = bf.ndarray.copy(self.block_N_2, space = 'cuda')
                ...
                for beam_num in range(3):
                    ...
                    for box_car in range(8):
                        if box_car == 0:
                            ...
                            tscrunched = odata[0, (3 * ob_index) + beam_num, :, :].copy(space = 'system')
